            ...
            <main id="mainContent" class="padFixBoth">
                ...
                <form id="" class="animated fadeIn margin-al-inputs formBrand">
                    <div class="row">
                        ...
                        <div class="col-4">
                            <input type="text" class="al-inputs required" v-model="brand.name" placeholder="Nombre de la marca" id="">
                        </div>

                        <div class="col-4">
                            <select data-select="on" class="required" data-name-input="Status Evento" name="status_event" v-model="brand.status" id="status_agente">
                                <option :value="0">Status de la marca</option>
                ...
                <div class="row">
                    <DataTable id="guest_table" :columns="columns" :data="listBrands" :options="dataTableOptions" ref="table" class="table table-hover table-striped view" width="100%">
                        <thead>
                        <tr>
                            <th>Name</th>
                            <th>Status</th>
...
const onDelete = async (id) => {
    const resp = await brandService.deleteOne(id);
    console.log(resp);
    await getBrand();
}
...
const onSave = async () => {
    ...
    if (respVal) {
        if (modificar.value) {
            resp = await brandService.updateOne(brand.value.id, brand.value);
        } else {
            resp = await brandService.saveOne(brand.value);
        }
        if (resp) {
...
<style scoped>
#brand-component .al-inputs, #brand-component .formSubmit {
    margin: 0;
}
